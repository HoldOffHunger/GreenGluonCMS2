<?php
	...
	elseif ($this->script_format_lower == 'html')
	{
		...
		$header_primary_args = [
			...
			'level'=>1,
			'divclass'=>'width-100percent border-2px margin-top-5px background-color-gray13',
			'textclass'=>'margin-0px padding-top-22px margin-bottom-22px',
	...
	$entryheader = new module_entryheader(['that'=>$this, 'time_frame'=>$time_frame]);
		...
		if($this->entry['image'])
		{
			...
			if($image_count)
			{
				...
				if(!$this->mobile_friendly)
				{
					$image_loop = $image_count;
					
					if($image_loop > 5) {
						$image_loop = 5;
					}
					
					for($i = 0; $i < $image_loop; $i++)
					{
						$image = $images[$i];
		...
		if($this->children && $children_count) {
			...
			$children_display = $entrysort->Sort(['entries'=>$this->children]);
