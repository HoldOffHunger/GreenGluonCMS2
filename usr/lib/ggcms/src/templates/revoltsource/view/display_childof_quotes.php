		...
		$auth->Display();
			...
			foreach($child_display as $child)
			{
				...
				foreach($child['children'] as $grandchild) {
					$goodtext = trim($grandchild['textbody'][0]['FirstThousandCharacters']);
					$goodtext_pieces = explode("\n", $goodtext);
					$goodtext = $goodtext_pieces[0];
					$goodtext = str_replace('<p', '<p style="margin:2px;"', $goodtext);
					print('<div style="margin:2px;" class="border-2px background-color-gray15 float-left">');
					print('<span class="horizontal-left margin-0px font-family-arial">');
					print('<i><a href="' . $child['Code'] . '/' . $grandchild['Code'] . '/view.php">');
					print($goodtext);
						
						# BT: associated field is missing???  fixplz
			#		print("<PRE>");
			#		print_r($grandchild);
			#		print("</PRE>");
