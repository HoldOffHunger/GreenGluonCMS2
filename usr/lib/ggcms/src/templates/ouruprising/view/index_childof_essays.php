<?php
	...
	$count = count($this->record_list);
	if($this->entry['ChildAdjective']) {
		$sub2_text .= $this->entry['ChildAdjective'];
		$sub2_text .= ' ';
		$sub2_text .= $parent['ChildNounPlural'];
	}elseif($count !== 0 && array_key_exists($count-2, $this->record_list)) {
		$parent = $this->record_list[$count-2];
		$adjective = $parent['GrandChildAdjective'];
		$noun_plural = $parent['GrandChildNounPlural'];
		
		$sub2_text .= $parent['GrandChildAdjective'];
		$sub2_text .= ' ';
		$sub2_text .= $parent['GrandChildNounPlural'];
	}
	
	$sub2_text .= ' : ' . $this->children_count;
	...
	$social_media_share_links_args = [
		...
		'socialmediasharelinkargs'=>[
			'url'=>$this->domain_object->GetPrimaryDomain(['insecure'=>1, 'lowercase'=>1, 'www'=>1]) . '/anarchism/view.php?action=index',
