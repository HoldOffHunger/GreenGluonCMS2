	...
	for($i = 0; $i < count($this->children); $i++)
	{
		...
		if($grandchildren) {
			...
			if($grandchildren_count) {
				print('<div class="horizontal-center" style="width:95%;">');
				for($j = 0; $j < $grandchildren_count; $j++) {
					...
					if($grandchildren_count > 1) {
						...
						print('<a href="' . $child['Code'] . '/street-art/' . $grandchild['Code'] . '/view.php?action=index">');
						print('<img width="');
					...
					if($great_grandchildren_count !== 0) {
						$entry_children->Display_Entries([
							'entries'=>$great_grandchildren,
							'count'=>$great_grandchildren_count,
							'custom_width'=>'30%',
							'url_action'=>'index',
						#	'datefield'=>'OriginalCreationDate',
							'url_prefix'=>$child['Code'] . '/' . $grandchild['Code'] . '/',
						]);
					}
				}
