<?php
		...
		# UTF-8 TEST: 利用規約の目的と目的
		# DO NOT PROCEED SAVING unless you save Japanese text above!
		# ----------------------------------------------------------
	...
	class terms extends view {
		...
		public function getLimitedLiabilitySection($args) {
			...
			switch($this->handler->language->getLanguageCode()) {
				...
				case 'de':
					...
					$limited_liability_paragraphs[] =
						'<p>In keinem Fall haften ' . $this->handler->domain->host . ' und seine leitenden Angestellten, Direktoren und Mitarbeiter für irgendetwas, das sich aus Ihrer Nutzung dieser Website ergibt oder in irgendeiner Weise damit zusammenhängt, ob eine solche Haftung vertraglich besteht. ' . $this->handler->domain->host . ', einschließlich seiner leitenden Angestellten, Direktoren und Mitarbeiter, haftet nicht für indirekte, Folgeschäden oder besondere Haftung, die sich aus Ihrer Nutzung dieser Website ergeben oder in irgendeiner Weise damit zusammenhängen.</p>';
					break;
				...
				case 'fr':
					...
					$limited_liability_paragraphs[] =
						'<p>En aucun cas, ' . $this->handler->domain->host . ', ni aucun de ses dirigeants, administrateurs et collaborateurs, ne pourra être tenu pour responsable de quoi que ce soit qui découle de ou qui est lié à votre utilisation de ce site Web, que cette responsabilité soit contractuelle ou non. ' . $this->handler->domain->host . ', y compris ses dirigeants, administrateurs et collaborateurs, ne saurait être tenu pour responsable de toute responsabilité indirecte, consécutive ou particulière résultant de ou liée à votre utilisation de ce site Web.</p>';
					break;
				...
				case 'ja':
					...
					$limited_liability_paragraphs[] =
						'<p>いかなる場合においても、' . $this->handler->domain->host . '、ならびにその役員、取締役、および共同編集者は、そのような責任が契約下にあるかどうかにかかわらず、このWebサイトの使用から生じるまたはこれに関連する何らかの方法について責任を負いません。 役員、取締役、および共同編集者を含む' . $this->handler->domain->host . 'は、このWebサイトの使用に起因する、またはそれに関連する間接的、結果的、または特別な責任について一切責任を負いません。</p>';
					break;
				...
				case 'it':
					...
					$limited_liability_paragraphs[] =
						'<p>In nessun caso ' . $this->handler->domain->host . ', né alcuno dei suoi funzionari, direttori e collaboratori, potrà essere ritenuto responsabile di qualsiasi conseguenza derivante da o in qualsiasi modo connessa con l\'uso di questo Sito Web, indipendentemente dal fatto che tale responsabilità sia contrattualizzata. ' . $this->handler->domain->host . ', compresi i suoi funzionari, direttori e collaboratori, non potrà essere ritenuta responsabile di alcuna responsabilità indiretta, consequenziale o speciale derivante da o in qualsiasi modo correlata al tuo utilizzo di questo sito web.</p>';
					break;
				...
				case 'nl':
					...
					$limited_liability_paragraphs[] =
						'<p>' . $this->handler->domain->host . ' en haar functionarissen, directeurs en medewerkers zullen in geen geval aansprakelijk worden gehouden voor iets dat voortvloeit uit of in enig opzicht verband houdt met uw gebruik van deze Website, ongeacht of deze aansprakelijkheid onder contract valt. ' . $this->handler->domain->host . ', inclusief haar functionarissen, directeurs en medewerkers, kan niet aansprakelijk worden gesteld voor enige indirecte, vervolg- of speciale aansprakelijkheid die voortvloeit uit of in enig opzicht verband houdt met uw gebruik van deze website.</p>';
					break;
				...
				case 'pl':
					...
					$limited_liability_paragraphs[] =
						'<p>W żadnym wypadku ' . $this->handler->domain->host . ', ani żaden z jej członków, dyrektorów ani współpracowników nie będzie ponosił odpowiedzialności za jakiekolwiek skutki wynikające z korzystania z Witryny lub w jakikolwiek sposób związane z korzystaniem z tej witryny internetowej, niezależnie od tego, czy taka odpowiedzialność wynika z umowy. ' . $this->handler->domain->host . ', w tym jej funkcjonariusze, dyrektorzy i współpracownicy nie ponoszą odpowiedzialności za jakiekolwiek pośrednie, wynikowe lub szczególne zobowiązania wynikające z lub w jakikolwiek sposób związane z korzystaniem z tej strony internetowej.</p>';
					break;
				...
				case 'pt':
					...
					$limited_liability_paragraphs[] =
						'<p>Em nenhum caso a ' . $this->handler->domain->host . ', nem qualquer de seus diretores, diretores e colaboradores, será responsabilizada por qualquer coisa decorrente ou de alguma forma relacionada ao seu uso deste Website, independentemente de tal responsabilidade estar sob contrato. A ' . $this->handler->domain->host . ', incluindo seus diretores, diretores e colaboradores, não será responsabilizada por nenhuma responsabilidade indireta, conseqüente ou especial decorrente ou de qualquer forma relacionada ao uso deste site.</p>';
					break;
				...
				case 'ru':
					...
					$limited_liability_paragraphs[] =
						'<p>Ни в коем случае ' . $this->handler->domain->host . ', ни один из его должностных лиц, директоров и сотрудников не будет нести ответственность за все, что вытекает из или каким-либо образом связано с использованием вами данного Веб-сайта, независимо от того, является ли такое обязательство договорным. ' . $this->handler->domain->host . ', включая его должностных лиц, директоров и сотрудников, не несет ответственности за какую-либо косвенную, косвенную или специальную ответственность, вытекающую из или каким-либо образом связанную с использованием вами данного веб-сайта.</p>';
					break;
				...
				case 'tr':
					...
					$limited_liability_paragraphs[] =
						'<p>Hiçbir durumda, ' . $this->handler->domain->host . ' ne de herhangi bir memuru, yöneticisi ve ortak çalışanı, bu Web Sitesini kullanmanızla ilgili olarak herhangi bir şekilde veya herhangi bir şekilde bu yükümlülüğün sözleşmeye tabi olup olmadığına ilişkin herhangi bir şeyden sorumlu tutulamaz. ' . $this->handler->domain->host . ', memurları, yöneticileri ve işbirlikçileri de dahil olmak üzere, bu web sitesini kullanmanızdan kaynaklanabilecek herhangi bir şekilde dolaylı, dolaylı veya özel sorumluluktan sorumlu tutulamaz.</p>';
					break;
				...
				case 'zh':
					...
					$limited_liability_paragraphs[] =
						'<p>在任何情况下，' . $this->handler->domain->host . '及其任何高级职员，董事和合作者均不对因使用本网站而产生的任何方式承担任何责任，无论此类责任是否属于合同。 ' . $this->handler->domain->host . '，包括其官员，董事和合作者，对因使用本网站而产生或以任何方式引起的任何间接，后果性或特殊责任概不负责。</p>';
					break;
			}
		...
		public function getImageBaseDirectory() {
			...
			if(	($this->script_format_lower === 'pdf') ||
				...
				$base_directory = 'https://www.' . $this->handler->domain->host . '.com' . $base_directory;
			}
