	...
	class style extends basicscript {
		...
		public function IsAccessible() {
			if($this->script_extension === 'css') {
				if(!$this->format->OneCSSFilePerPage()) {
						// functionalize
				...
				//	print(" (((" . $requested_css_url_base . ")))");
					...
					foreach([$this->handler->domain->host, 'default'] as $option) {
						$script_css_location = GGCMS_DIR . 'templates/' . $option . '/' . $this->object_code . '/' . $this->script_file . '_css.php';
	...
	#						'margin-right'=>'auto',
		...
		public function Display_Attributes_GetPixelOrPercentage() {
			...
			if($last_two_chars === 'px') {
				...
				$pixel_count = $this->handler->cleanser->CleanseInput_Integer($cleanse_count_args)['cleansedinput'];
				...
				if($last_seven_chars === 'percent') {
					...
					$percent_count = $this->handler->cleanser->CleanseInput_Integer($cleanse_count_args)['cleansedinput'];
