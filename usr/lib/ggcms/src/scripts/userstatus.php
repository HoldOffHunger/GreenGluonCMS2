<?php
	...
	class userstatus extends view {
		...
		public function SetComments_GetRecords() {
			...
			$unapproved_comment_records = $this->handler->db_access->GetRecords($comment_get_args);
			...
			foreach($comments as $comment_key => $comment) {
				...
				if($decision) {
					...
					if($new_accept != $comment['Approved'] || $new_reject != $comment['Rejected']) {
						...
						$new_comment = $this->handler->db_access->UpdateRecord($comment_update_args);
		...
		public function SetRejectedComments_GetRecords() {
			...
			$rejected_comment_records = $this->handler->db_access->GetRecords($comment_get_args);
			...
			if($submission) {
				...
				if($host_name && $date_range) {
					...
					if($host_name == 'allhosts') {
						$statistics_location = GGCMS_DIR . 'stats/';
						...
						$statistics_location = GGCMS_DIR . 'stats/' . $this->host_name . '/';
