		...
		public function SetSiteMap_EntryScriptLinks($args) {
			...
			if($entry_code_count) {
				for($i = 0; $i < $entry_code_count; $i++) {
					...
					$new_sitemap_item = '';
					$url = $this->handler->domain->GetPrimaryDomain(['lowercase'=>1, 'www'=>1]) . '/' . implode('/', $entry_code_list) . '/view.php';
	...
	#		print_r($entry_codes);
		...
		public function SetSiteMap_GetEntries($args) {
			...
			$links = [
				[
					'url'=>[
						'loc'=>$this->handler->domain->GetPrimaryDomain(['lowercase'=>1, 'www'=>1]) . '/',
				...
				[
					'url'=>[
						'loc'=>$this->handler->domain->GetPrimaryDomain(['lowercase'=>1, 'www'=>1]) . '/about.php',
...
#				print_r($languages_last_mod);
				$links[] = [
					'url'=>[
						'loc'=>$this->handler->domain->GetPrimaryDomain(['lowercase'=>1, 'www'=>1]) . '/languages.php',
			...
			foreach($this->handler->language->GetListOfLanguageCodes() as $language_code => $language) {
				...
				if($contact_language_list[$language_code]) {
					$links[] = [
						'url'=>[
							'loc'=>$this->handler->domain->GetPrimaryDomain(['lowercase'=>1, 'www'=>1]) . '/contact.php?language=' . $language_code,
							'lastmod'=>$contact_last_mod,
							'changefreq'=>$contact_change_freq,
							'priority'=>$contact_priority,
						],
		...
		public function GetHTMLFormatData_Title() {
			if(!$this->parent['id'] && $this->master_record && $this->master_record['id']) {
				if($this->handler->language->getLanguageCode() === 'en') {
					...
					if($sitemap_header_language_translations[$this->handler->language->getLanguageCode()]) {
						$header_title_text = $sitemap_header_language_translations[$this->handler->language->getLanguageCode()];
					} else {
						$header_title_text = 'Sitemap for ' . $this->master_record['Code'] . ' : ' . $this->master_record['Subtitle'];
					}
				}
