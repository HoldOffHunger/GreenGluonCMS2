	...
	class languageutils extends basicscript {
		...
		public function display() {
			...
			if(strlen($this->letter) > 0) {
				$file_location = GGCMS_DIR . 'classes/Language/Words/EnglishMisspellings/EnglishMisspellingsWords_' . strtoupper($this->letter) . '.php';
				...
				foreach($new_words as $correct => $incorrect) {
					...
					foreach($incorrect as $misspelling => $truth_value) {
						if($correct != $misspelling) {
							$real_misspellings[] = $misspelling;
						}
					}
					
					if(count($real_misspellings) > 0) {
						$new_text .= "\t\t\t\t'" . $correct . "'=>[\n";
						
						ksort($incorrect);
						foreach($real_misspellings as $misspelling) {
