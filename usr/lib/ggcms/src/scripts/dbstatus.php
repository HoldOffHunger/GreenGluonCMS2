<?php
	...
	class dbstatus extends basicscript {
				...
				#	print("BT: CONNECT COMPLETE.");
					...
					if($client_db) {
						if($client_db->DBStart()) {
							...
							if($lookup_lists && count($lookup_lists) > 0) {
		...
		public function DetectEntryCodeLength() {
			if(!$this->Param('fix')) {
				return TRUE;
			}
		...
		public function EntriesMissingListTitles() {
			$where = 'WHERE Entry.ListTitle = ""';
			
			return $this->EntriesMissingAnyField(['where'=>$where]);
		}
		
		public function EntriesMissingListTitleSortKeys() {
			$where = 'WHERE Entry.ListTitleSortKey = ""';
			
			return $this->EntriesMissingAnyField(['where'=>$where]);
		}
		
		public function EntriesMissingChildAdjectives() {
			$where = 'WHERE Entry.ChildAdjective = ""';
			
			return $this->EntriesMissingAnyField(['where'=>$where]);
		}
		
		public function EntriesMissingChildNouns() {
			$where = 'WHERE Entry.ChildNoun = ""';
			
			return $this->EntriesMissingAnyField(['where'=>$where]);
		}
		
		public function EntriesMissingChildNounsPlural() {
			$where = 'WHERE Entry.ChildNounPlural = ""';
			
			return $this->EntriesMissingAnyField(['where'=>$where]);
		}
		
		public function EntriesMissingGrandChildAdjectives() {
			$where = 'WHERE Entry.GrandChildAdjective = ""';
			
			return $this->EntriesMissingAnyField(['where'=>$where]);
		}
		
		public function EntriesMissingGrandChildNouns() {
			$where = 'WHERE Entry.GrandChildNoun = ""';
			
			return $this->EntriesMissingAnyField(['where'=>$where]);
		}
		
		public function EntriesMissingGrandChildNounsPlural() {
			$where = 'WHERE Entry.GrandChildNounPlural = ""';
			
			return $this->EntriesMissingAnyField(['where'=>$where]);
		}
		
		public function EntriesMissingAnyField($args) {
			if(!$this->Param('fix')) {
				return TRUE;
			}
			...
			if($entry_level === 1) {
				$parent_extra_where = $args['where'];
			}
			
			$entries = $this->getPrimaryEntries(['extrawhere'=>$parent_extra_where]);
